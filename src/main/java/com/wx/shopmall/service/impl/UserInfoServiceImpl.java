...
public class UserInfoServiceImpl implements UserInfoControllerService {
    ...
    public String weChatLogin(String code) {
        try {
            if (ObjectUtil.isNotEmpty(code) && "".equals(code)){
                throw  new Exception("code不能为空");
            }else{
                //登录参数获取的业务实现
                JSONObject weChatSession = getWeChatSession(code, url, appSecret, appId);
                logger.info(weChatSession.toString());
                //wechat返回的openId
                String  openId = weChatSession.getString("appid");
                //wechat返回的sessionKey
    ...
    public static JSONObject getWeChatSession(String code,String url , String appId , String appSecret){
        ...
        String jsonString = HttpClientUtils.doGetForWeChat(url, requestUrlParam);
